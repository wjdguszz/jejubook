...
select * from jeju where temano=4;

select count(title) from jeju where temano=3;

SELECT * FROM JEJU WHERE TITLE='애월';

select DESCRIPTION from jeju where title='엄태웅, 한가인 주연의 영화-건축학개론';
update jeju set description='순수한 첫사랑의 추억을 간직하고 싶은 많은 사람들에 의해 오픈세트장이 2013년 4월 카페 "서연의 집"으로 재탄생 하였다. 현재 제주로 여행 온 많은 관광객들이 "서연의 집"에서 커피와 함께 첫사랑의 기억을 떠올리며 힐링하고 있는 중이다.';
